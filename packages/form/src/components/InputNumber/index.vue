<template>
  <div class="starfish-formitem"
    ...
    <div class="control" :style="{ marginLeft: labelalign != 'top' ? labelWidth + 'px' : '' }">
      <el-input-number v-model="item.data.default" :controls-position="item.data.type == 2 ? 'right' : ''" :size="size"
        v-if="drag" />
      <el-input-number v-model="data[item.data.fieldName]" v-if="!drag"
        :controls-position="item.data.type == 2 ? 'right' : ''" :size="size" @focus="execFunc('onFocus')"
